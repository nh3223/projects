...
const vestingData = [
  ...
  {
    grantDate,
    originalVestingDate: stringify(new Date('February 1, 2022')),
    acceleratedVestingDate: transactionDate,
  ...
  {
    grantDate,
    originalVestingDate: stringify(new Date('March 1, 2022')),
    acceleratedVestingDate: transactionDate,
...
test('should have one element with an original vesting date of January 1 with the same new vesting date and 100 shares', () => {

  const oldVestingDate = stringify(new Date('January 1, 2022'))
  expect(convertedVestingData[0].originalVestingDate).toEqual(oldVestingDate);
  expect(convertedVestingData[0].newVestingDate).toEqual(oldVestingDate);
  expect(convertedVestingData[0].shares).toEqual(100);

});

test('should have one element with an original vesting date of February 1 with a different new vesting date and 100 shares', () => {

  const oldVestingDate = stringify(new Date('February 1, 2022'))
  expect(convertedVestingData[1].originalVestingDate).toEqual(oldVestingDate);
  expect(convertedVestingData[1].newVestingDate).toEqual(transactionDate);
  expect(convertedVestingData[1].shares).toEqual(100);

});

test('should have two elements with an original vesting date of March 1, with two different new vesting dates with 50 shares each', () => {

  const oldVestingDate = stringify(new Date('March 1, 2022'))
  expect(convertedVestingData[2].originalVestingDate).toEqual(oldVestingDate);
  expect(convertedVestingData[3].originalVestingDate).toEqual(oldVestingDate);
