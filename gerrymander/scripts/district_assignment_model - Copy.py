...
class DistrictAssignmentProcessingAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        number_of_districts = 16 # NEED TO FIGURE OUT HOW TO IMPORT # OF DISTRICTS TO AUTOMATE DIFFERENT STATES
...
class District:
    ...
    '''
    def get_potential_swaps(self, neighboring_district):
        potential_swap_tracts = []
        border_tracts = self.get_border_tracts(neighboring_district)
        neighbor_border_tracts = neighboring_district.get_border_tracts(self)
        border_average_tract_distance = self.get_average_tract_distance(border_tracts)
        neighbor_border_average_tract_distance = neighboring_district.get_average_tract_distance(border_tracts)
        for tract in border_tracts:
            district_distance = Utilities.get_distance(tract.center, self.center)
            neighbor_district_distance = Utilities.get_distance(tract.center, neighboring_district.center)
            swap_ratio = ((district_distance / border_average_tract_distance) / (neighbor_district_distance / neighbor_border_average_tract_distance)) - 1
            potential_swap_tracts.append({
                'tract': tract,
                'swap_ratio': swap_ratio
            })
        potential_swap_tracts.sort(key = lambda d: d['swap_ratio'], reverse = True)
...
class InitialAssignment:
    ...
    def run_assignment(self):
        for district in self.districts:
            starting_tract = max(self.get_available_tracts())
            district.initialize(starting_tract)
            while district.population < self.target_district_population:
                tract_to_be_assigned = self.nearest_available_neighbor(district)
                if tract_to_be_assigned:
                    Utilities.assign(district, tract_to_be_assigned)
                else:
                    new_starting_tract = self.get_new_starting_tract(district)
    ...
    def nearest_available_neighbor(self, district):   
    ...
    def get_available_tracts(self):
        return [tract for tract in self.census_tracts if tract.district is None]
    
    def get_new_starting_tract(self, district):
        available_tracts = self.get_available_tracts()
...
class ConsolidateDistricts:
    ...
    def consolidate_districts(self):
        ...
        def get_replacement_tract(district, new_district):
            district.sort_neighbors()
            while len(district.neighbors) > 0:
                tract = district.get_nearest_neighbor()
                if tract.district == new_district:
                    return tract
            return None
        ...
        self.show_feedback(str(consolidated_districts))
...
class CompactDistricts:
    ...
    def __init__(self, model_parameters, show_feedback):
        ...
        self.border_tracts = self.get_border_tracts()
        
        self.compact_districts()
        Utilities.relocate_centers(self.districts)
        self.show_feedback('Compact Districts Complete')
    
    def old_compact_districts(self):
        number_swaps = 0
        for district in self.districts:
            district.relocate_center()
            neighboring_districts = district.get_neighboring_districts()
            for neighboring_district in neighboring_districts:
                district_potential_swaps = district.get_potential_swaps(neighboring_district)
                neighboring_district_potential_swaps = neighboring_district.get_potential_swaps(district)
                swaps = zip(district_potential_swaps, neighboring_district_potential_swaps)
                for swap in swaps:
                    district_potential_swap = swap[0]
                    neighbor_potential_swap = swap[1]
                    if district_potential_swap['swap_ratio'] + neighbor_potential_swap['swap_ratio'] > 0:
                        number_swaps += 1
                        self.swap_tracts(district_potential_swap['tract'], neighbor_potential_swap['tract'])
    ...
    def compact_districts(self):
        ...
        def get_distance_ratios(district, neighboring_district, tracts):
            ...
            for tract in tracts:
                ...
                if district_distance_ratio < 1 and district_distance_ratio < neighboring_district_distance_ratio:
                    distance_ratio = {
                        'tract': tract,
        ...
        for district in self.districts:
            ...
            for neighboring_district in neighboring_districts:
                ...
                if neighbor_tracts and border_tracts:
                    ...
                    #self.show_feedback(str(neighbor_tract_distance_ratios))
                    #self.show_feedback(str(border_tract_distance_ratios))
...
class PopulationAdjustment:
    ...
    def adjust_overpopulation(self, district, assigned_districts):
        district.set_border_tracts()
        district.set_average_border_tract_distance()
        neighboring_districts = self.get_neighboring_districts(district, assigned_districts)
        available_neighbors = []
        for neighboring_district in neighboring_districts:
            adjacent_neighbors = self.get_adjacent_neighbors(district, neighboring_district)
            if adjacent_neighbors:
                available_neighbors.extend(self.get_adjacent_neighbors(district, neighboring_district))
            ...
            def find_neighbors(feature, features, index):
                ...
                for id in intersecting_ids:
                    ...
                    if intersecting_feature != feature and not intersecting_feature.geometry().disjoint(feature.geometry()):
                        neighbors.append(intersecting_feature['GEOID_Data'])
                return neighbors
